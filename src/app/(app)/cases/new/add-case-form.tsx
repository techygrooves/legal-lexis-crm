...
export function AddCaseForm({
  ...
  // Default to picking an existing client when any exist — most users land
  // here after adding the client first. Fall back to the new-client form when
  ...
  const selectedExisting = existingClients.find(
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (clientMode === "existing" && !existingClientId) {
      toast.error("Pick a client", {
        description: "Choose one of your existing clients or switch to New client.",
      });
      return;
    }
    ...
    const result = await createCase({
      existingClientId: clientMode === "existing" ? existingClientId : "",
      client:
        clientMode === "new"
          ? {
          ...
          : { fullName: "", email: "", phone: "", address: "", source: "" },
      caseDetails: {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {clientMode === "existing"
                ? "Pick one of your existing clients — a client can have multiple cases."
                : "A new client record will be created for this case."}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div
              role="radiogroup"
              aria-label="Client selection mode"
              ...
              <button
                ...
                className={
                  clientMode === "existing"
                    ...
                    : "rounded-md px-3 py-1.5 text-sm text-muted-foreground hover:text-foreground disabled:opacity-50"
                }
              >
                Existing client
              </button>
            ...
            {clientMode === "existing" ? (
              existingClients.length === 0 ? (
                <p className="rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-700 dark:bg-amber-950 dark:text-amber-400">
                  No clients yet. Switch to <strong>New client</strong> to create
                  one with this case, or{" "}
                  <Link
                    href="/clients/new"
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div className="sm:col-span-2">
                    <Field id="existing-client" label="Client">
                      <Select
                  ...
                  {selectedExisting && (selectedExisting.email || selectedExisting.phone) && (
                    <p className="text-xs text-muted-foreground sm:col-span-2">
                      {selectedExisting.email}
                      {selectedExisting.email && selectedExisting.phone ? " · " : ""}
